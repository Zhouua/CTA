...
| 序号 | 触发任务 | 事项 | 状态 |
|---|---|---|---|
| Q1 | T2 | xlsx 软重复列裁决：见下表 7 对，等用户指定保留 / 删除哪一列（脚本不会自动删）。`cleaned` 目录目前保留全部列，未影响后续任务，但若要在 T3/T4 之前先消歧，请在此表里把"决定"一列填完。 | **等待用户** |
| Q2 | T3.3* | 哑变量是否保留（依赖 ablation 结果） | 待 T4.2 完成 |

**Q1 软重复明细**（corr 绝对值、NaN 重叠率；列号对应 audit 文档）：

| Product | Col A | 指标 A (ID) | Col B | 指标 B (ID) | \|corr\| | NaN 重叠 | 决定（待用户） |
|---|---:|---|---:|---|---:|---:|---|
| AU | 2 | 库存:黄金 (S000025737) | 3 | 期货库存:黄金 (S000025741) | 1.000 | 1.000 | – |
| AU | 5 | COMEX:黄金:总持仓 (S002825231) | 7 | COMEX:黄金:期货(新版):总持仓:持仓数量 (S008523183) | 1.000 | 1.000 | – |
| JM | 1 | 生产资料价格:焦煤(1/3焦煤) (M003575153) | 2 | 市场价:焦煤(主焦煤):当旬值 (S005948590) | 1.000 | 0.987 | – |
| M | 3 | 现货价:豆粕 (S004242412) | 5 | 现货价:豆粕:地区均价 (S002856699) | 0.997 | 0.910 | – |
| RB | 5 | 期货库存:螺纹钢:小计 (S005476450) | 6 | 注册仓单:螺纹钢 (S002853768) | 1.000 | 1.000 | – |
| RU | 6 | 仓单数量:天然橡胶:总计 (S004410360) | 8 | 仓单数量:天然橡胶 (S002842061) | 1.000 | 1.000 | – |
| SN | 1 | 库存:锡:总计 (S005580375) | 2 | 期货库存:锡:总计 (S005580468) | 0.995 | 1.000 | – |
