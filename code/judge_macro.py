...
对外接口：
    build_monthly_regime(macro_csv_path, lag_months=1) -> pd.DataFrame
        返回以月初日期为索引的 DataFrame，包含：
            ...
            open_position         : bool  - 两者之一为 True 则允许开仓
...
def _series_with_lag(raw: pd.Series, dates: pd.Series, lag_months: int) -> pd.Series:
    ...
    """
    lagged = raw.shift(lag_months).astype("boolean").fillna(False).astype(bool)
    lagged.index = dates
    return lagged
...
def build_monthly_regime(
    ...
    regime["open_position"] = regime["is_macro_strong"] | regime["is_inventory_strong"]
    return regime
